...
export class ValidatorService {
  ...
  addValidationRule(target: any, propertyKey: string, rule: IValidationRule) {
    let rules: PropertyValidationList =
      Reflect.getMetadata('validation', target, propertyKey) || new PropertyValidationList()
    rules.addRule(rule)
  ...
  validate(target: any) {
    ...
    if (Array.isArray(keys)) {
      for (const key of keys) {
        const propertyValidationList = Reflect.getMetadata(
          'validation',
          target,
          key
        ) as PropertyValidationList
